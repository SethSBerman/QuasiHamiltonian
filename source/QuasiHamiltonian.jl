...
function funcMatrix(A::Array{<:Real})
    return Real[rep(A) zeros(Real, 3, 3);zeros(Real, 3, 3) transpose(rep(inv(A)))]
end

function getValue(j, k)
    f(A) = A[j,k]
    return f
end

function pointTangent(x1::Real, x2::Real, x3::Real, y1::Real, y2::Real, y3::Real, K)
...
function infAction(a::Real,b::Real,c::Real)
